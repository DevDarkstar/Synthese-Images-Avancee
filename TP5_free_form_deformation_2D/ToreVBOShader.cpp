...
void computeBernsteinPols()
{
  ...
  {
    // Calcul des polynômes de Bernstein pour les axes x et y du point du cercle unitaire courant
...
void updateCircleVerticesPosition()
{
  ...
  for(int i = 0; i < sommets_cercle.size(); i++)
  {
    ...
    for(int t = 0; t < 3; t++)
    {
      for(int s = 0; s < 3; s++)
      {
        // Récupérations des polynômes de bernstein aux instants s et t
        ...
        pos[0] += (s_coord * t_coord * control_point[0]);
        pos[1] += (s_coord * t_coord * control_point[1]); 
        pos[2] += (s_coord * t_coord * control_point[2]);
      }
    }
...
//----------------------------------------
{
  ...
  glutCreateWindow("TP5 - FREE FORM DEFORMATION 2D");
