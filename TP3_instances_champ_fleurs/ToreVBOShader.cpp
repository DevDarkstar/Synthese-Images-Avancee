...
#define FOLIAGE_INSTANCES 2000 // Nombre d'instances de végétation
...
GLuint VBO_sommets_vegetation, VBO_indices_vegetation, VBO_texture_index, VAO_vegetation;
...
// location des VBO
//------------------
GLuint indexVertex=0, indexUVTexture=2, indexNormale=3, indexTexture=4;
...
void createFoliageSupport()
{
  for(int i = 0; i < FOLIAGE_PLANE_NUMBER; i++){
    ...
    sommets_support[i*4*8 + 1] = 2*SUPPORT_RADIUS;
    sommets_support[i*4*8 + 2] = SUPPORT_RADIUS * sin(angle);
    ...
    // deuxième point
    sommets_support[i*4*8 + 8] = - SUPPORT_RADIUS * cos(angle);
    sommets_support[i*4*8 + 9] = 2*SUPPORT_RADIUS;
    sommets_support[i*4*8 + 10] = - SUPPORT_RADIUS * sin(angle);
    ...
    // troisième point
    sommets_support[i*4*8 + 16] = -SUPPORT_RADIUS * cos(angle);
    sommets_support[i*4*8 + 17] = 0.0f;
    sommets_support[i*4*8 + 18] = -SUPPORT_RADIUS * sin(angle);
    ...
    // quatrième point
    sommets_support[i*4*8 + 24] = SUPPORT_RADIUS * cos(angle);
    sommets_support[i*4*8 + 25] = 0.0f;
    sommets_support[i*4*8 + 26] = SUPPORT_RADIUS * sin(angle);
...
void getUniformLocationFoliage(FoliageIDs& foliage){
  ...
  foliage.programID = LoadShaders("FoliageShader.vert", "FoliageShader.frag");
...
void genereVBOFoliageSupport(void){
  ...
  if(glIsBuffer(VBO_texture_index) == GL_TRUE) glDeleteBuffers(1, &VBO_texture_index);
  glGenBuffers(1, &VBO_texture_index);
  glBindBuffer(GL_ARRAY_BUFFER, VBO_texture_index);
  glBufferData(GL_ARRAY_BUFFER, sizeof(foliage_texture_index),foliage_texture_index , GL_STATIC_DRAW);
  ...
  glEnableVertexAttribArray(indexTexture); // location = 4
  ...
  glBindBuffer(GL_ARRAY_BUFFER, VBO_texture_index);
  glVertexAttribPointer(indexTexture, 1, GL_FLOAT, GL_FALSE, sizeof(GLfloat), (void*)0);
  // On indique que l'on doit mettre à jour la valeur de l'attribut indexTexture toutes les nouvelles instances
  glVertexAttribDivisor(indexTexture, 1);
...
void deleteVBOFoliageSupport()
{
  ...
  glDeleteBuffers(1, &VBO_texture_index);
